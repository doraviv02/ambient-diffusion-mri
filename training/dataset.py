...
class MultiViewKspaceDataset(torch.utils.data.Dataset):
    precond = "ambient_mv"

    def __init__(self, path, base_R=4, acs_lines=16, delta_fraction=0.3,
                 min_noise_variance=1e-8, cache=False, seed=0, **_ignore):
        from utils.train_masks import base_and_delta, random_cartesian_mask
        self._base_and_delta = base_and_delta
        self._random_mask = random_cartesian_mask
        self._path = path
        self.base_R = base_R
        ...
        if os.path.isdir(path):
            self._files = sorted(_glob.glob(os.path.join(path, "*.pt")))
        else:
            raise IOError(f"MultiViewKspaceDataset path must be a directory: {path}")
        if not self._files:
            raise IOError(f"No .pt files in {path}")
        probe = self._load_processed(0)
        self._resolution = int(probe["ksp_views"].shape[-1])
        self._num_coils = int(probe["ksp_views"].shape[1])
        self.name = os.path.basename(os.path.normpath(path))

    # attributes read by training_loop
    @property
    def resolution(self):
    ...
    @property
    def num_channels(self):
        return 2  # complex image (real/imag)
    ...
    def _load_processed(self, idx):
        ...
        bundle = torch.load(self._files[idx], map_location="cpu", weights_only=False)
        proc_path = bundle.get("processed_path", self._files[idx])
        proc = torch.load(proc_path, map_location="cpu", weights_only=False)
        if self._cache_enabled:
            self._cache[idx] = proc
        return proc

    def __getitem__(self, idx):
        proc = self._load_processed(idx)
        ksp = proc["ksp_views"]          # [V,C,H,W] complex, fully sampled
        maps = proc["s_maps_views"]      # [V,C,H,W] complex (shared)
        noise = proc["noise_std"]        # [V]
        meta = proc.get("metadata", {})
        V, C, H, W = ksp.shape
        g = np.random.RandomState(int(torch.randint(0, 2 ** 31 - 1, (1,)).item()))
        a = int(g.randint(0, V)); b = a
        while b == a:
            b = int(g.randint(0, V))
        in_mask, in_delta = self._base_and_delta(H, W, self.base_R, self.acs_lines,
                                                 self.delta_fraction, g)
        tgt_mask, _, _ = self._random_mask(H, W, self.base_R, self.acs_lines, g)
        return {
            "input_ksp": ksp[a], "input_mask": in_mask, "input_delta_mask": in_delta,
            "input_maps": maps[a],
            "target_ksp": ksp[b], "target_mask": torch.from_numpy(tgt_mask),
            "target_maps": maps[b],
            "input_noise_std": noise[a], "target_noise_std": noise[b],
            "subject_id": str(meta.get("subject_id", f"idx{idx}")),
            "slice_id": int(meta.get("slice_id", idx)),
        }